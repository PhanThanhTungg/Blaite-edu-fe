...
export async function POST(request: NextRequest) {
  ...
  if (!knowledge) return NextResponse.json({ error: 'Knowledge not found or not allowed' }, { status: 404 });

  let finalContent = content;
  if (typeof finalContent !== 'string' || !finalContent.trim()) {
    // Nếu không có content, tự động sinh câu hỏi từ knowledge.content
    try {
      const gen = await generateQuestionWithGemini({ content: knowledge.content });
      finalContent = gen.question;
    } catch (e: any) {
      return NextResponse.json({ error: e.message || 'Failed to generate question' }, { status: 500 });
    }
    if (!finalContent) {
      return NextResponse.json({ error: 'Could not generate question from knowledge' }, { status: 500 });
    }
  }
...
export async function PUT(request: NextRequest) {
  ...
  if (!questionId || typeof answer !== 'string' || !answer.trim()) {
    return NextResponse.json({ error: 'questionId và answer là bắt buộc' }, { status: 400 });
  }
  ...
  if (!question) return NextResponse.json({ error: 'Question not found or not allowed' }, { status: 404 });
